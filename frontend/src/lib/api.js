...
export const api = {
  // An app's screens. The order is the app's navigation order, so these are
  // replaced wholesale rather than patched.
  appViews: (app) =>
    callMethod(
      'oneapp_control.api.admin.app_views',
      { app },
      {
        silent: true,
        method: 'GET',
  ...
  siteState: (tenant) =>
    callMethod(method('admin.site_state'), { tenant }, { silent: true, method: 'GET' }),
  siteJobs: (tenant) =>
    callMethod(method('admin.site_jobs'), { tenant }, { silent: true, method: 'GET' }),
  siteBackups: (tenant) =>
    callMethod(method('admin.site_backups'), { tenant }, { silent: true, method: 'GET' }),
  siteDomains: (tenant) =>
    callMethod(method('admin.site_domains'), { tenant }, { silent: true, method: 'GET' }),
  supportLogins: (tenant) =>
    callMethod(method('admin.support_logins'), { tenant }, { silent: true, method: 'GET' }),

  takeBackup: (tenant) =>
    callMethod(
      method('admin.take_backup'),
      { tenant },
      {
  ...
  setPrimaryDomain: (tenant, domain) =>
    callMethod(
      method('admin.set_primary_domain'),
      { tenant, domain },
      {
  ...
  pushBenchConfig: (shard) =>
    callMethod(
      'oneapp_control.provisioning.bench_config.push_to_shard',
      { shard },
      {
  ...
  pushBenchConfigAll: () =>
    callMethod(
      'oneapp_control.provisioning.bench_config.push_to_all_shards',
      {},
      {
