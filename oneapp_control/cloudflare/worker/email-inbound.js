...
var BOUNCE_PREFIX = "t-";
var LOCAL = /^[a-z0-9](?:[a-z0-9._+-]*[a-z0-9+])?$/;
...
function parseRecipient(recipient, mailDomain) {
  ...
  if (local.startsWith(BOUNCE_PREFIX)) {
    const tenant2 = local.slice(BOUNCE_PREFIX.length);
    return isSlug(tenant2) ? { tenant: tenant2, localPart: "", bounce: true } : null;
  }
...
var index_default = {
  async email(message, env, ctx) {
    ...
    const payload = {
      ...
      text: parsed.text || "",
      html: parsed.html || "",
      attachments: collectAttachments(parsed, Number(env.MAX_ATTACHMENT_BYTES || 25e6))
    };
